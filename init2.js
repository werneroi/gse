...
firebase.auth().onAuthStateChanged(function (user) {
  ...
  if (user) {
    ...
    firebase.database().ref('/users/' + userId).once('value').then(function (snapshot) {
      ...
      console.log("looking for google sync: ",currentUser)
      if (currentUser.googleSync == true) {
       console.log("found google sync on");
        googleSync = true;
        googleCall = "init"
      ...
      firebase.database().ref("calUpdates/").orderByChild("timestamp").startAt(Date.now()).on("child_changed", function (snapshot) {
        console.log("a change value in calUpdates: ", JSON.stringify(snapshot));
       });
      firebase.database().ref("calUpdates/").orderByChild("timestamp").startAt(Date.now()).on("child_added", function (snapshot) {
        console.log("added value in calUpdates: ", JSON.stringify(snapshot));
       });
      ...
      firebase.database().ref("updates/" + currentUser.appId).on("child_added", function (snapshot) {
       // console.log("a new value in updates: ", JSON.stringify(snapshot.val().sender));
       // console.log("a new value in updates key: ", JSON.stringify(snapshot.key));
...
function ReadInfoOnce() {
 ...
 // console.log("getting updates once now for: ", currentUser.appId);
  ...
  firebase.database().ref("/inBox/" + currentUser.appId + "/all/").orderByChild('timestamp').once("value", function (snapshot) {
      ...
      document.getElementById('badgeNum').innerText = "Inbox " + (snapshot.numChildren());
   ...
   console.log("a new inBox message details!", snapshot.val());
    ...
    if (googleSync == true) {
     ...
     console.log("Message status: ",snapshot.val().status)
      ...
      else if(snapshot.val().status == "waiting meeting deleted"){
...
firebase.database().ref("messages").on("child_changed", function (snapshot) {
  ...
  //// console.log("a change snap: ",JSON.stringify(snapshot.val().timestamp));
 // console.log("a change snap creator: ", snapshot.val().creator);
 // console.log("a change snap to: ", snapshot.val().to);
 ...
 // console.log('general listener !! ');  
  ...
  allUsersSnapShot.forEach(function (child) {
    if (child.val().appId == currentUser.appId) {
     ...
     // console.log("!!!!!!!!!!!!!!!CURRENT USER CAL ARRAY UPDATED!!!!!!!!!!!!!!")
    }
